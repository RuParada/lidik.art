...
if( !class_exists( 'dttheme_privacy_class' ) ) {
	...
	class dttheme_privacy_class {
		...
		function __construct() {
			...
			$privacy_mcfrm = redart_option('privacy','enable-mailchimpfrm-msg');
			...
			$privacy_loginfrm = redart_option('privacy','enable-loginfrm-msg');
		...
		function dttheme_privacy_comment_checkbox_content( $content = "", $extra_class = "" ) {
			...
			if( empty($content) ) $content = do_shortcode( redart_option('privacy', 'commentfrm-msg') );
		...
		function dttheme_privacy_verify_comment_checkbox( $commentdata ) {
			...
			if($post_type!= 'product'){
				...
				if ( ! is_user_logged_in() && ! isset( $_POST['comment-form-dt-privatepolicy'] ) ) {
					$error_message = apply_filters( 'dttheme_privacy_comment_checkbox_error_message', __( 'Error: You must agree to our privacy policy to comment on this site...' , 'dtthemes-core' ) );
		...
		function dttheme_privacy_mailchimp_checkbox( $content = "", $attrs ) {
			...
			if( empty($content) ) $content = do_shortcode( redart_option('privacy', 'mailchimpfrm-msg') );
		...
		function dttheme_privacy_login_extra( $form ) {
			...
			$content = do_shortcode( redart_option('privacy', 'loginfrm-msg') );
		...
		function dttheme_authenticate_user_acc( $user, $password ) {
		    ...
		    if ( isset( $_REQUEST['comment-form-dt-privatepolicy'] ) ) {
		        ...
		        $error = new WP_Error();
		        $error->add('did_not_accept', __( 'You must acknowledge and agree to the privacy policy' , 'dtthemes-core'));
		        return $error;
		    }
		}
		...
		function dttheme_print_tracking_code() {
			...
			$temp = redart_option('pageoptions', 'tracking-code');
			...
			$enable_code = redart_option('pageoptions', 'enable-tracking-code');
...
if( ! function_exists( 'dttheme_cookie_consent' ) ) {
    ...
    function dttheme_cookie_consent() {
		...
		if( isset( $cookie_bar ) ) {		
			...
			$message  = redart_option('privacy','cookiebar-msg');
			$position = redart_option('privacy','cookiebar-position'); 
            ...
            <div class="dt-cookie-consent cookiebar-hidden dt-cookiemessage-<?php echo esc_attr($position); ?>">
	            <div class="container">
					...
					$buttons = redart_option('privacy-bar');
					$i = 0;
					$extra_info = "";

					if( empty( $buttons ) ):
						?><a href="#" class="dt-sc-button filled small dt-cookie-consent-button dt-cookie-close-bar" data-contents="<?php echo esc_attr($cookie_contents); ?>"><?php esc_html_e('OK', 'dtthemes-core'); ?></a><?php
					endif;
					...
					if( !empty( $buttons ) ):
						foreach($buttons as $button) {
							...
							if( !empty( $button['action'] ) && $button['action'] == 'info_modal' ) {
								...
								$contents[] = array(	'label'		=> __( 'Other external services', 'dtthemes-core' ), 
				...
				[dt_sc_privacy_video_embeds]', 'dtthemes-core' )
								);

	
								$wp_privacy_page = get_option('wp_page_for_privacy_policy');
								if( !empty( $wp_privacy_page ) ) {
									$contents[] = array(	'label'		=> __( 'Privacy Policy', 'dtthemes-core' ), 
															'content'	=> __( 'You can read about our cookies and privacy settings in detail on our Privacy Policy Page. <br><br> [dt_sc_privacy_link]', 'dtthemes-core' ));
								}


								$custom_model = redart_option('privacy','enable-custom-model');
								if( isset( $custom_model ) ) {
									$contents = redart_option('privacy-model');
									$heading  = str_replace("'", "&apos;", redart_option('privacy', 'custom-model-title'));
								}
