...
class DTCoreShortcodesDefination {
	...
	function dt_sc_woo_cart($attrs, $content = null) {
		...
		if( function_exists("is_woocommerce")) :
		    $out = '<a class="cart-info '.$class.'" href="'.wc_get_cart_url().'" title="'.esc_attr__( 'View Shopping Cart', 'dtthemes-core' ).'">';
	...
	function dt_sc_portfolios( $attrs, $content = null ){
		...
		if($the_query->have_posts()):
			...
			#Ajax load more...
			if($ajax_loadmore == 'yes'):
			$out .= '   <div class="aligncenter load-more">';
			$out .= '		  <div class="aligncenter">';
			$out .= '			   <a id="ajax_load_gallery" href="javascript:void(0)" class="loadmore dt-sc-button medium with-icon icon-left filled dt-sc-expand dt-sc-expand-horizontal" data-per-page="'.esc_attr($post_per_page).'" data-taxonomy="'.implode(',', $cats).'" data-item-class="'. esc_attr($post_class).'" data-allow-space="'. esc_attr($allow_gridspace).'" data-style="'. esc_attr($style).'" data-galleryloadmore-nonce="'. wp_create_nonce('galleryloadmore_nonce').'" data-allow-filter="'.esc_attr($ajax_loadmore).'"><span class="fa fa-picture-o"></span>'. esc_html__('Load More', 'redart').'<i class="load-icon"></i></a>';
			$out .= '	   </div>';
			$out .= '  </div>';
			endif;
		...
		$out = '<div class="dt-sc-events-list-wrapper">';
			...
			if( class_exists('Tribe__Events__Main') || class_exists('Tribe__Events__Pro__Main') ):
				if(!empty($title))	$out .= "<h3>{$title}</h3>";
					...
					$events = tribe_get_events( array( 'eventDisplay' => 'all', 'posts_per_page' => $limit,
	...
	function dt_sc_special_events_list( $attrs, $content = null ){
		...
		if( class_exists('Tribe__Events__Main') || class_exists('Tribe__Events__Pro__Main') ):
			$events = tribe_get_events( array( 'eventDisplay' => 'all', 'posts_per_page' => $limit, 'tax_query'=> array( array( 'taxonomy' => 'tribe_events_cat', 'field' => 'id', 'terms' => $cats ) ) ));
			if($events): $i = 1;
				switch($type):
	...
	function dt_sc_privacy_disable_google_tracking( $attrs = array() , $content = "") {	
		$content = !empty($content) ?  $content : __('Click to enable/disable google analytics tracking.', 'dtthemes-core');
	...
	function dt_sc_privacy_disable_google_webfonts( $attrs = array() , $content = "") {
		$content = !empty($content) ?  $content : __('Click to enable/disable google webfonts.', 'dtthemes-core');
	...
	function dt_sc_privacy_disable_google_maps( $attrs = array() , $content = "") {	
		$content = !empty($content) ?  $content : __('Click to enable/disable google maps.', 'dtthemes-core');
	...
	function dt_sc_privacy_disable_video_embeds( $attrs = array() , $content = "") {	
		$content = !empty($content) ?  $content : __('Click to enable/disable video embeds.', 'dtthemes-core');
	...
	function dt_sc_google_map( $attrs, $content = null ) {
		...
		$api_key = redart_option('layout','googlemap-api-key');
